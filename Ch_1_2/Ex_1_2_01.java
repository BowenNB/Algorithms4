...
public class Ex_1_2_01 {
  ...
  public static void minDist(int N) {
    ...
    StdDraw.setPenRadius(0.1); //这里的setPenRadius()方法是设置画笔的粗细，参数是double类型的，表示画笔的粗细，值越大，画笔越粗
    Point2D[] points = new Point2D[N];
    for (int i = 0; i < N; i++) {
      points[i] = new Point2D(StdRandom.uniform(), StdRandom.uniform());//Point2D是algs4库中提供的二维点类，用于表示二维平面上的点;StdRandom.uniform()返回一个0到1之间的随机数
      points[i].draw();
    }
